...
from photosocial.models import User, Role, Tag, Photo, Comment
from photosocial.decorators import permission_required, admin_required
from photosocial.utils import redirect_back
from photosocial.forms.admin import EditProfileAdminForm
...
def index():
    ...
    return render_template('admin/index.html', photo_count=photo_count, reported_photos_count=reported_photos_count,
                           user_count=user_count, locked_user_count=locked_user_count,
                           block_user_count=blocked_user_count, tag_count=tag_count, comment_count=comment_count,
...
def manage_photo(order):
    ...
    per_page = current_app.config['PHOTOSOCIAL_MANAGE_PHOTO_PER_PAGE']
    order_rule = '被举报次数'
    if order == 'by_time':
...
def manage_user():
    ...
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['PHOTOSOCIAL_MANAGE_USER_PER_PAGE']
...
def manage_tag():
    """管理标签"""
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['PHOTOSOCIAL_MANAGE_TAG_PER_PAGE']
...
def manage_comment(order):
    """管理评论"""
    page = request.args.get('page', 1, type=int)
    per_page = current_app.config['PHOTOSOCIAL_MANAGE_COMMENT_PER_PAGE']
    order_rule = '被举报次数'
    if order == 'by_time':
